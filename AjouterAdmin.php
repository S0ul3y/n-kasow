...
if (isset($_POST['enregistrer'])) {
    if(!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['email']) && !empty($_POST['role']) && !empty($_POST['pass'])){
    // if (isset($_POST['nom'], $_POST['prenom'], $_POST['email'], $_POST['pass'])) {
        ...
        if ($result > 0) {
            ...
            $req = $bd->prepare('INSERT INTO client (nom, prenom, email,role,mdp) VALUES (?, ?, ?,?, ?)');
            $req->bindValue(1, $_POST['nom']);
            $req->bindValue(2, $_POST['prenom']);
            $req->bindValue(3, $email);
            $req->bindValue(4, $_POST['role']);
            $req->bindValue(5, $mdpHash);
